...
/* --- Sidebar --- */
function Sidebar({ activeTab, onTabChange, unreadCount, pendingConfirmations }: { activeTab: TabKey; onTabChange: (t: TabKey) => void; unreadCount: number; pendingConfirmations: number }) {
  const sidebarNav = getSidebarNav(unreadCount, pendingConfirmations)
  return (
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    api.get('/products/pending-confirmation')
      .then((res) => setPendingConfirmations(res.data.products?.length || 0))
      .catch(() => setPendingConfirmations(0))
  }, [activeTab])
