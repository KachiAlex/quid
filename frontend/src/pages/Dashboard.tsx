...
import {
  Home, LayoutDashboard, Bell, AlertTriangle, Repeat, Shield,
  CreditCard, Target, Lightbulb, Users, ArrowUpRight,
  RefreshCw, User, ScanLine,
  TrendingUp, Lock, Car, Wifi, Zap as ZapIcon, Layers,
  CheckCircle2, ArrowRight, Crown, MessageSquare,
  Eye, Bot,
} from 'lucide-react'
...
const sidebarNav = [
  ...
  { label: 'Alerts', icon: AlertTriangle, href: '/dashboard', badge: 5 },
  { label: 'Switches', icon: Repeat, href: '/dashboard' },
  { label: 'Quid Shield', icon: Shield, href: '/dashboard' },
  { label: 'Transactions', icon: CreditCard, href: '/dashboard' },
...
const bottomNav = [
  ...
  { label: 'Alerts', icon: AlertTriangle, href: '/dashboard', badge: 5 },
  { label: 'Scan', icon: ScanLine, href: '/dashboard', highlight: true },
  { label: 'Switches', icon: Repeat, href: '/dashboard' },
    ...
    <aside className="hidden lg:flex w-64 flex-col border-r border-white/5 bg-[#0d0d1a] px-4 py-6">
      ...
      <nav className="flex-1 space-y-1">
        {sidebarNav.map((item) => (
          <Link
            key={item.label}
            to={item.href}
            className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${item.active ? 'bg-[#7c3aed] text-white' : 'text-white/60 hover:bg-white/5 hover:text-white'}`}
          >
      ...
      <div className="flex items-center gap-2 rounded-xl border border-white/5 bg-white/5 px-3 py-2.5">
        ...
        <div>
          ...
          <p className="text-[10px] text-white/40">Bank-level encryption · Open Banking standards</p>
        </div>
      </div>
    </aside>
  )
      ...
      <div className="mx-auto flex max-w-md items-center justify-around">
        {bottomNav.map((item) => (
          <Link
            key={item.label}
            to={item.href}
            className={`relative flex flex-col items-center gap-0.5 rounded-xl px-3 py-1.5 text-[10px] font-medium transition ${item.highlight ? 'text-[#7c3aed]' : item.label === 'Home' ? 'text-white' : 'text-white/50'}`}
          >
            {item.highlight ? (
  ...
  return (
    <div className="mb-6 flex flex-col gap-4 sm:mb-8 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex items-center gap-3">
        ...
        <button className="relative flex h-9 w-9 items-center justify-center rounded-full bg-white/5 text-white/60 transition hover:bg-white/10 hover:text-white">
          <Bell className="h-4 w-4" />
          <span className="absolute right-0 top-0 h-2.5 w-2.5 rounded-full bg-rose-500 ring-2 ring-[#0a0a14]" />
        </button>
  ...
  return (
    <div className="mb-6 sm:mb-8">
      <div className="mb-4 flex items-center justify-between sm:mb-5">
        ...
        <Link to="/dashboard" className="text-xs font-medium text-[#a78bfa] transition hover:text-[#7c3aed]">See all (12)</Link>
      </div>
...
/* --- Middle Row: Quid Shield + Health Score --- */
function MiddleRow() {
  return (
...
/* --- Bottom Row: Hidden Money + AI Coach + Autopilot --- */
function BottomRow() {
  return (
  ...
  return (
    <div className="mb-6 sm:mb-8">
      <div className="mb-4 flex items-center justify-between sm:mb-5">
        ...
        <Link to="/dashboard" className="text-xs font-medium text-[#a78bfa] transition hover:text-[#7c3aed]">See all</Link>
      </div>
